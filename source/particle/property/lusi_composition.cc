...
namespace aspect
{
  namespace Particle
  {
	...
	if (in_a_top_cell && (part_compo_props[mrb_oc_crust_idx] > 0.1 || part_compo_props[ssz_oc_crust_idx] > 0.1 ) ) 
	  {
	    ...
	    part_compo_props[felsic_amphibolites_idx]=
	     std::max(0.0,std::min(0.45,part_compo_props[felsic_amphibolites_idx]));
            part_compo_props[felsic_granulites_idx]=
	     std::max(0.0,std::min(0.45,part_compo_props[felsic_granulites_idx]));
	}
	...
	// --- (p,T) conditions for which upwelling partially melted SSZ asth. transforms to SSZ oc. lith. mantle
        //     2025-09-06: removed the dependency on the vertical velo magnitude -> create_new_olm_ssz
	if ( (asth2SSZOlmPTTri1.ptInside(pressureInMPa_here,temperature_here) ||
	      asth2SSZOlmPTTri2.ptInside(pressureInMPa_here,temperature_here)) && !in_extension_stage ) //&& create_new_olm_ssz)
	  {
            ...
            // --- Avoid transformation of pm_frac to pm_ssz_asth_mtl_idx at the
            //     beginning of the convergence stage when part_compo_props[pm_ssz_asth_mtl_idx] is less than 0.001 
            if (part_compo_props[pm_ssz_asth_mtl_idx] > 0.001)
              {
                lusiMaterialChange(part_compo_props, pm_frac_idx, pm_ssz_asth_mtl_idx, 0.0, 1.0);
              }
