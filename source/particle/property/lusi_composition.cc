  ...
  namespace Particle
  {
	...
	if (part_compo_props[oc_seds_idx] > 0.5 && in_extension_stage) {
	  part_compo_props[acc_tot_strain_idx]=
        ...
        // --- cooled asth. mantle -> hybrid asth. mantle that as the olm th. diff
	...
	else // --- heated hybrid (non convective) asth. (if any) -> (convective) asth. 
	  {
	...
	if (xPositionMeters <= NO_MTC_ON_DISTANCE_FROM_SIDES ||
	  ...
	  // --- Do not do any mat. changes here (apart from the asth. to hyb. asth or vice-versa ) , just returns here.
	  return;
	}
